import {B as t, $ as e} from "./index-f45448ab.js";
const s = (t,e)=>{
    if ("height" !== e && "width" !== e)
    ...
    const s = "width" === e ? t.clientWidth : t.clientHeight;
    return ("width" === e ? t.scrollWidth : t.scrollHeight) > s
}
  , i = t=>{
    const e = Math.random().toString(36).substr(2, 9);
    return document.getElementById(t + e) ? i(t) : e
}
;
customElements.define("web-tabs", class extends t {
    ...
    render() {
        if (!this.prerenderedChildren) {
            ...
            for (const s of this.children) {
                this.limit && t === this.limit + 1 && (this.tabs.push(this.loadMoreTab()),
                this.prerenderedChildren.push(e`<div class="web-tabs__panel" role="tabpanel" hidden></div>`)),
                this.prerenderedChildren.push(this.panelTemplate(t, s));
                const i = s.getAttribute("data-label");
                this.tabs.push(this.tabTemplate(t, i)),
                t++
            }
        }
        return e`
    ...
    onClickLoadMore(t) {
        ...
        e.parentElement.querySelectorAll(".web-tabs__tab.hidden").forEach((t=>t.classList.remove("hidden"))),
    ...
    updated(t) {
        t.has("activeTab") && this._changeTab()
    }
    _changeTab() {
        const t = this.querySelectorAll(".web-tabs__tab")
          , e = this.querySelectorAll(".web-tabs__panel")
          , s = t[this.activeTab]
          , i = e[this.activeTab];
        if (s) {
            for (const e of t)
    ...
    onResize() {
        const t = this.querySelector(".web-tabs__tablist");
        this.overflow = s(t, "width")
    }
    onFocus(t) {
        const e = t.currentTarget
          , s = this.querySelectorAll(".web-tabs__tab")
          , i = Array.from(s).indexOf(e);
        e.scrollIntoView({
            behavior: "smooth",
            block: "nearest",
            inline: "center"
    ...
    previousTab() {
        const t = this.querySelectorAll(".web-tabs__tab");
        t[this.activeTab - 1] ? this.activeTab = this.activeTab - 1 : this.activeTab = t.length - 1
    }
    nextTab() {
        const t = this.querySelectorAll(".web-tabs__tab");
        this.activeTab = (this.activeTab + 1) % t.length || 0
    }
    firstTab() {
        this.activeTab = 0
    }
    lastTab() {
        const t = this.querySelectorAll(".web-tabs__tab");
        this.activeTab = t.length - 1
    }
    indexOfTabByChild(t) {
        const e = t.closest('[class="web-tabs__panel"]');
        if (!this.contains(e))
            return -1;
        const s = parseInt(e.getAttribute("data-index"));
        return isNaN(s) ? -1 : s
    }
}
);
customElements.define("web-copy-code", class extends t {
    constructor() {
        super(),
    ...
    onCopy() {
        ...
        t.selectNode(this.querySelector("code")),
